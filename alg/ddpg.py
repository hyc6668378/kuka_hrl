...
class DDPG(object):
    ...
    def _setup_model(self, rank, memory_size, alpha,
                     obs_space, a_space, noise_target_action, **kwargs):
        ...
        with self.graph.as_default():
            ...
            if self.use_prioritiy:
                ...
                self.memory = Memory(limit=memory_size, action_shape=a_space.shape,
            ...
            self.action_memory = tf.placeholder(tf.float32, [None] + list(a_space.shape), name='actions_from_memory')
            ...
            with tf.variable_scope('Actor'):
                self.action= self.build_actor(self.normalized_observe_Input,
                                               scope='eval', trainable=True, a_space=a_space)
                self.action_ = self.build_actor(self.normalized_observe_Input_,
                                                scope='target', trainable=False, a_space=a_space)
            ...
            with tf.variable_scope('Critic'):
                ...
                self.q_1 = tf.clip_by_value(
                    self.build_critic(self.normalized_observe_Input, self.action,
                                      scope='eval_1', trainable=True),
                    self.Q_value_range[0], self.Q_value_range[1])

                q_1_ = self.build_critic(self.normalized_observe_Input_, noised_action_,
                ...
                if self.use_TD3:
                    ...
                    q_2_ = self.build_critic(self.normalized_observe_Input_, noised_action_,
    ...
    def Save(self):
        # 只存权重,不存计算图.
        self.saver.save(self.sess, save_path="model/" + self.experiment_name + "/model.ckpt")
    ...
    def store_transition(self,
        ...
        if demo:
            self.memory.append_demo( obs0=obs0, action=action, reward=reward,
                                     obs1=obs1, terminal1=terminal1)
        else:
            self.memory.append( obs0=obs0, action=action, reward=reward,
                            obs1=obs1, terminal1=terminal1)
